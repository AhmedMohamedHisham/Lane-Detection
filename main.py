...
def draw_lane(undistorted, binary_warped, left_fit, right_fit, left_curverad, right_curverad):
    ...
    cv2.putText(result, "Center Lane Position: " + "{:0.2f}".format(offset) + 'm', org=(50, 50),
                fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                fontScale=1, color=(255, 255, 255), lineType=cv2.LINE_AA, thickness=2)
    cv2.putText(result, "Curvature Radius: " + "{:0.2f}".format((left_curverad + right_curverad) / 20) + 'm',
                org=(50, 100), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                fontScale=1, color=(255, 255, 255), lineType=cv2.LINE_AA, thickness=2)
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if str1 == 'P' or str1 == 'p':
            ...
            undistorted = cv2.undistort(img, mtx, dist, None, mtx)
            binary_warped, binary_threshold = thresholding(undistorted, M)
            left_fit, right_fit, left_curved, right_curved, _ = sliding_window(binary_warped)
            result = draw_lane(undistorted, binary_warped, left_fit, right_fit, left_curved, right_curved)
            result = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)
            cv2.imshow('Lane Detection of Photo', result)
            cv2.waitKey(0)
        elif str1 == 'V' or str1 == 'v':
            str2 = input('Do you want to record the output video, "Y" for yes or "N" for No ?\n')
            clip = VideoFileClip("project_video.mp4")
            driving_clip = clip.fl_image(get_processor(15))
            driving_clip.preview()
            cv2.destroyAllWindows()
            # driving_clip.ipython_display()
            if str2 == 'Y' or str2 == 'y':
                movie_output = 'output/Output_project_video.mp4'
                pr = cProfile.Profile()
                pr.enable()
